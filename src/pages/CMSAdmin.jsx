...
import { COLOR_PRESETS, HEADING_FONTS, BODY_FONTS } from '../lib/themeOptions';
...
const SECTIONS = ['Tema', 'Brand', 'Hero', 'About', 'Menu', 'Galeri', 'Fitur', 'Testimoni', 'Kontak', 'Footer', 'Ulasan Pengunjung', 'Statistik'];
    ...
    <div className="min-h-screen bg-[#F9F7F3] font-sans text-[#2C241B]">
      ...
      <div className="max-w-5xl mx-auto px-6 py-8 flex flex-col md:flex-row gap-8">
        ...
        <div className="flex-1 bg-white rounded-lg border border-[#2C241B]/10 p-6">
          ...
          {activeSection === 'Tema' && (
            <div>
              <h2 className="font-serif text-2xl mb-6">Tema Web</h2>

              <p className="text-sm font-medium text-[#2C241B] mb-3">Preset Warna</p>
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 mb-8">
                {COLOR_PRESETS.map((preset) => {
                  const isActive =
                    content.theme.colorBg === preset.colorBg &&
                    content.theme.colorDark === preset.colorDark &&
                    content.theme.colorDarker === preset.colorDarker;
                  return (
                    <button
                      key={preset.name}
                      onClick={() => {
                        update('theme.colorBg', preset.colorBg);
                        update('theme.colorDark', preset.colorDark);
                        update('theme.colorDarker', preset.colorDarker);
                      }}
                      className={`text-left border rounded p-3 transition-colors ${
                        isActive ? 'border-[#2C241B] ring-1 ring-[#2C241B]' : 'border-[#2C241B]/15 hover:border-[#2C241B]/40'
                      }`}
                    >
                      <div className="flex gap-1.5 mb-2">
                        <span className="w-6 h-6 rounded-full border border-black/10" style={{ backgroundColor: preset.colorBg }} />
                        <span className="w-6 h-6 rounded-full border border-black/10" style={{ backgroundColor: preset.colorDark }} />
                        <span className="w-6 h-6 rounded-full border border-black/10" style={{ backgroundColor: preset.colorDarker }} />
                      </div>
              ...
              <p className="text-sm font-medium text-[#2C241B] mb-3">Warna Custom</p>
              <div className="space-y-3 mb-8">
                {[
                  { key: 'colorBg', label: 'Warna Background (terang)' },
                  { key: 'colorDark', label: 'Warna Utama (teks, tombol)' },
                  { key: 'colorDarker', label: 'Warna Footer (paling gelap)' },
                ].map(({ key, label }) => (
                  <div key={key} className="flex items-center gap-3">
                    <input
                      ...
                      className="w-12 h-10 rounded border border-[#2C241B]/20 cursor-pointer shrink-0"
                    />
                    <div className="flex-1">
                      <p className="text-sm">{label}</p>
                      <p className="text-xs text-[#2C241B]/40">{content.theme[key]}</p>
                    </div>
                  </div>
                ))}
              </div>

              <p className="text-sm font-medium text-[#2C241B] mb-3">Font Judul</p>
              <select
                value={content.theme.fontHeading}
                onChange={(e) => update('theme.fontHeading', e.target.value)}
                className="w-full border border-[#2C241B]/20 rounded px-3 py-2 text-sm mb-6"
              >
                {HEADING_FONTS.map((f) => (
              ...
              <p className="text-sm font-medium text-[#2C241B] mb-3">Font Teks Isi</p>
              <select
                value={content.theme.fontBody}
                onChange={(e) => update('theme.fontBody', e.target.value)}
                className="w-full border border-[#2C241B]/20 rounded px-3 py-2 text-sm mb-2"
              >
                {BODY_FONTS.map((f) => (
                  <option key={f.value} value={f.value}>{f.value}</option>
                ))}
              ...
              <p className="text-xs text-[#2C241B]/40 mb-6">
                Simpan dulu buat lihat perubahan font/warna di web utama (CMS sengaja gak ikut berubah, biar tetap konsisten buat admin).
              </p>
            </div>
          )}
