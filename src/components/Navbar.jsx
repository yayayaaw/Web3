...
// EDIT DI SINI: nama brand, menu navigasi, teks tombol reservasi
const Navbar = () => {
    ...
    >
      <div className="max-w-7xl mx-auto px-6 md:px-12 flex justify-between items-center">
        ...
        <div className={`hidden md:flex items-center space-x-10 text-sm tracking-widest uppercase ${isScrolled ? 'text-[#2C241B]' : 'text-white'}`}>
          <a href="#about" className="hover:opacity-60 transition-opacity">Tentang</a>
          <a href="#menu" className="hover:opacity-60 transition-opacity">Menu</a>
          <a href="#gallery" className="hover:opacity-60 transition-opacity">Galeri</a>
          <a href="#location" className="hover:opacity-60 transition-opacity">Lokasi</a>
        </div>
      ...
      <div className={`md:hidden absolute top-full left-0 w-full bg-[#F9F7F3] border-b border-[#2C241B]/10 overflow-hidden transition-all duration-500 ease-in-out ${isMobileMenuOpen ? 'max-h-96 opacity-100' : 'max-h-0 opacity-0'}`}>
        <div className="px-6 py-8 flex flex-col space-y-6 text-center text-[#2C241B]">
          <a href="#about" onClick={() => setIsMobileMenuOpen(false)} className="text-lg font-serif tracking-wide">Tentang Kami</a>
          <a href="#menu" onClick={() => setIsMobileMenuOpen(false)} className="text-lg font-serif tracking-wide">Menu Makanan & Minuman</a>
          <a href="#gallery" onClick={() => setIsMobileMenuOpen(false)} className="text-lg font-serif tracking-wide">Galeri Visual</a>
          <a href="#location" onClick={() => setIsMobileMenuOpen(false)} className="text-lg font-serif tracking-wide">Lokasi & Kontak</a>
